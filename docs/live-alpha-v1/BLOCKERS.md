...
|---|---|---|---|
| B1 (CLEARED 2026-09-24 11:17 UTC, owner ran the merges) | Merge to `main` needs the owner. | The agent's `gh pr merge` was refused by the session's permission classifier ("Merge Without Review"), 2026-09-24 10:1x UTC. | Berkay runs `gh pr merge <n> --merge` after reading the PR. Railway then deploys `main`. |
