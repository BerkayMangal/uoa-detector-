...
| 5.20 | **Study E** — can anything learnable rank next-session or five-session returns across the board's ten tickers? Pre-registered in `docs/study-E-signal-preregistration.md`, answered in `docs/study-E-result.md`: **REJECTED** against its own 200-pass shuffled-label null. Amended 2026-09-20 after the audit found its walk-forward had no purge: the harness was corrected, the null was rerun in full, and §1a reports what changed (horizon 1 a no-op on all 200 reps, horizon 5's real arm 3.2× lower). The published rejection was never in doubt because the leak could only inflate the arm that failed. Its two docs keep `study-E-*` names instead of the `preregister_<L>.md` pattern; the deviation is recorded here rather than silently extended to later letters. |
| 5.21 | **Study F** — does any learnable pattern on daily OHLCV rank next-session or five-session returns across a 70-name universe? Pre-registered in `docs/study-F-preregistration.md`, amended before the runner existed by `docs/study-F-preregistration-addendum.md` (a purge at the walk-forward boundary), answered in `docs/study-F-result.md`: **REJECTED** on both horizons against a 200-pass best-of-ten shuffled-label null. Keeps the `study-F-*` naming of 5.20 rather than the `preregister_<L>.md` pattern. Its market-neutral arm clears its own null at horizon 5 and is recorded as an observation, not a finding, because §7 fixes T1 on the forward label and T3 runs only after T1 holds — promoting it after the fact is what D4 forbids. |
| 5.22 | **Study G** — does a ranking learned from daily OHLCV predict the RESIDUAL five-session return? Pre-registered in `docs/study-G-preregistration.md`, frozen 2026-09-20 **before a single session of its data exists**. It exists because Study F's market-neutral arm cleared its own null at horizon 5 (+0.0505 against p99 +0.0426, 0 of 200 draws) while §7's order refused to let that be a finding. Measured the same day: UW's daily endpoint returns exactly 252 sessions, so there is no unsearched window on disk — the study is therefore scored **forward**, on sessions dated 2026-09-19 or later, out-of-sample by construction. First evaluation at 210 accumulated sessions (~2027-04), then 331 and 452; three checkpoints, dates fixed here rather than by the result. Features, models, protocol and null are reused from Study F unchanged, so the search does not widen. |
| 5.23+ | Further research studies (`preregister_<L>.md`, next letter H; non-burned windows only). **5.24 is carved out of this band** and is not a study letter; letters continue at 5.23 and skip 5.24 |
| 5.24 | **Options Alpha v1** (`options_alpha_v1`) — option-first candidate and PAPER signal engine, contract/spread construction, cost and risk gates, exit monitoring and outcome recording, plus the option-side research families. Progress in `docs/options-alpha-v1/`, artifacts in `artifacts/options-alpha-v1/`. Deliberately NOT placed in the reserved 5.10–5.19 engine-and-data band, whose named candidates belong to other work. Scope, data quality tier and the reasons behind each choice are in `docs/options-alpha-v1/DECISIONS.md` |
| 5.25 | **Live Alpha v1** (`live_alpha_v1`) — the live recommendation screen at `/` ("Bugünün Fırsatları"): reasoned BUY / CONDITIONAL_BUY / WATCH / AVOID / BEARISH_SETUP views from stored flow, spot, bars, UW news headlines and priced option alternatives, with an immutable record per decision and a PAPER tracker. Owner product override 2026-09-24 (`docs/live-alpha-v1/PRODUCT_OVERRIDE.md`); contract `docs/phase-5.25-live-alpha-acceptance.md`. Experimental rules, no measured edge. Carved out of the 5.23+ study band like 5.24; study letters skip 5.25 as well |
